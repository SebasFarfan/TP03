...
def obtenerDatosPagina(enlace):
    ...
    bloqueHeader=estructuraHTML.find('div',{'class':'article-info-wrapper'})
    listadoContenidoPagina = obtenerHeader(bloqueHeader)
    bloque = estructuraHTML.find('article',{'class':'note-detail notapage_minuto'})
    imagen=obtenerImagen(bloque)
    listadoContenidoPagina.append(imagen)
    contenido = estructuraHTML.find('div', {'class':'note-body newDetailTextChange clearfix'})
...
textoSinStopwords = operacionTexto.eliminarStopWords(textoTokenizado)
